...
public class SQLAlbumDAO implements AlbumDAO {
    ...
    private String sqlGetAllAlbumTracks = "SELECT tracks.* FROM albums INNER JOIN track_album ON albums.id=track_album.album_id INNER JOIN tracks ON track_album.track_id=tracks.id WHERE albums.id=?";
